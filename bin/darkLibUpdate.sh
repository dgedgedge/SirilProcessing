...
PY_SCRIPT="${SCRIPT_DIR}/darkLibUpdate.py"
...
if [[ -n "${VENV_DIR:-}" ]]; then
  ...
  echo "[darkLibUpdate] Aucun venv trouve."
...
if [[ ! -f "${SELECTED_VENV}/bin/activate" ]]; then
  echo "[darkLibUpdate] Script d'activation introuvable: ${SELECTED_VENV}/bin/activate"
  exit 1
fi
