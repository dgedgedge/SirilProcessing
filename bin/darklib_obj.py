#!/bin/env python3
"""
darklib_obj.py - DarkLib class definition
Extracted from darklib.py for better code organization
"""
...
# Import from darklib module (will be in the same directory)
# These imports will be resolved at runtime
# from darklib import FitsInfo, run_siril_script
...
class DarkLib:
    ...
    def group_dark_files(self, input_dirs, log_groups: bool = True, log_skipped: bool = False):
        """
        ...
        from darklib import FitsInfo
        
        # Validation: ensure input_dirs is not None and is iterable
        if input_dirs is None:
            logging.warning("input_dirs is None. Using empty list as default.")
            input_dirs = []
        elif not hasattr(input_dirs, '__iter__') or isinstance(input_dirs, str):
            logging.error(f"input_dirs must be an iterable (list or similar), got {type(input_dirs).__name__}")
            raise TypeError(f"input_dirs must be an iterable (list or similar), not {type(input_dirs).__name__}")
    ...
    def stack_and_save_master_dark(self, group_key: str, fitsinfo_list, process_dir: str, link_dir: str) -> None:
        """
...
"""
    ...
    def read_existing_master_darks(self):
        """
